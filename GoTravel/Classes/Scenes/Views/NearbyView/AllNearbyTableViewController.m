...
//@property(nonatomic,assign)CLLocationDegrees lastLatitude;
//@property(nonatomic,assign)CLLocationDegrees lastlongitude;
...
-(void)loadValue
{
  ...
  [NetWorkHandle getDataWithURLString:[NSString stringWithFormat:@"http://api.breadtrip.com/place/pois/nearby/?category=0&start=%ld&count=20&latitude=%f&longitude=%f",_indexStart,_latitude,_longitude] compare:^(id object) {
    ...
    [KDataBasehandle createtableName:@"allNearby" Modelclass:[NearbyTableViewModel class]];
      
      for (NSDictionary *dict1 in array)
      {
          self.model = [[NearbyTableViewModel alloc] initWithDictionary:dict1];
      NSDictionary *dic2 = dict1[@"location"];
          self.model.lat = dic2[@"lat"];
          self.model.lng = dic2[@"lng"];
          
          [self.dataArray addObject:self.model];
          
#pragma mark ----将网络下载的数据存入数据库
      
       [KDataBasehandle insertIntoTableModel:self.model];
...
-(void)tableView:(UITableView *)tableView didSelectRowAtIndexPath:(NSIndexPath *)indexPath
{
    ...
    AllNearbyDetailVC.view.backgroundColor = [UIColor whiteColor];
    UINavigationController *NV = [[UINavigationController  alloc] initWithRootViewController:AllNearbyDetailVC];
    
    AllNearbyDetailVC.modalPresentationStyle = UIModalPresentationOverCurrentContext;
...
-(void)loadRefresh
{
    ...
    __weak AllNearbyTableViewController *RVC = self;
